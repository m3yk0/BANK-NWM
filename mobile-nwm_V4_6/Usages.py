...
loca = localizar_arquivo(criar=True, test_open=True, text=False, dir='START-NWM/mobile-nwm_V4_6', arquivo='registro.xlsx')
nome_arquivo = loca[ 2 ]
...
def ver_contas(tipo_do_registro='Completo', nome_arquivo=nome_arquivo, contas=None, vars=False, view=True):
	"""
	...
	Args:
		...
		nome_arquivo: str(caminho do arquivo 'registro.xlsx'). Padrão==localizar_arquivo(criar=True, test_open=True, text=False, dir='START-NWM/mobile-nwm_V4_6', arquivo='registro.xlsx')[2]
		vars: bool
			...
			if 'none' not in secao and 'none' not in classes:
				if contas[ secao ].values().__len__() > 0:  # Caso seção tenha classes , então mostra:
					sect_part = [ part, f"""\n*●-  -=- {secao.upper()} -=-    -●*\n""",
								  ...
								  """ ፧⠂ *Taxa Selic do Reino:* {}\n""".format(
									  format_currency(float(moneypersec[ secao ] - PIB_Per_Capita_persec[ secao ]), '',
													  locale='pt_br')),
					...
					if 'contas em conj' not in secao:  # Caso a seção atual não seja 'contas em conjunto/NPC'
						secpart_rei = f""" ፧⠂ *Reinante:* {prevent(store.reinantes, secao)}\n"""
					...
					for classe, persons in classes.items():
						if persons != 'none' and contas[ secao ][ classe ].values().__len__() > 0 and [
							...
							if 'contas em conj' not in secao:  # Caso a seção atual não seja 'contas em conjunto/NPC'
								lider_part = f"*Líder:* {prevent(store.lideres, classe).title()}\n"
								sec_parts.append(lider_part)
							# print(lider_part, end='')
							for name, saldo in persons.items():
								if name.strip().lower() != 'none' and 'none' != saldo:
									name_part = f"\n- > {name.title():}: "
...
def cadastrar_conta(
	...
	Args:
		nome_arquivo: caminho + arquivo.xlsx (do excel) a qual o novo indivíduo será salvo. Padrão==localizar_arquivo(criar=True, test_open=True, text=False, dir='START-NWM/mobile-nwm_V4_6', arquivo='registro.xlsx')[2]
...
def verificar_presença(contas=None, registro=nome_arquivo, nome_sect=None, nome_cla=None, nome_pp=None,
	...
	Args:
		registro: str - patch file registro.xlsx. Commum==localizar_arquivo(criar=True, test_open=True, text=False, dir='START-NWM/mobile-nwm_V4_6', arquivo='registro.xlsx')[2]
...
def remover_conta(nome_arquivo=nome_arquivo, contas=None, save_in_archive=False, name=None, classe=None, section=None):
	...
	Args:
		...
		nome_arquivo: str = Caso 'contas=None', é pego objeto Contas novo diretamente do tal arquivo. Padrão==localizar_arquivo(criar=True, test_open=True, text=False, dir='START-NWM/mobile-nwm_V4_6', arquivo='registro.xlsx')[2] Padrão==localizar_arquivo(criar=True, test_open=True, text=False, dir='START-NWM/mobile-nwm_V4_6', arquivo='registro.xlsx')[2]
...
def add_classe(nome_arquivo=nome_arquivo, contas=None, save_in_archive=False, secao_selected=None, nome_novo=None):
	...
	Args:
		...
		nome_arquivo: str = Caso 'contas=None', é pego objeto Contas novo diretamente do tal arquivo. Padrão==localizar_arquivo(criar=True, test_open=True, text=False, dir='START-NWM/mobile-nwm_V4_6', arquivo='registro.xlsx')[2]
		contas: Caso contas != None, as operações não serão feitas com o registro em arquivo, mas sim com o objeto Contas, que pode já estar diferente do registro.xlsx (registro em arquivo). Mudança local
		save_in_archive: bool {True (Save) | False (Not save)}fará com que a mudança seja salva diretamente no arquivo.
...
def remover_secao(section=None, nome_arquivo=nome_arquivo, contas=None, save_in_archive=False):
	...
	if confirm(
			f" Você confirma a exclusão da seção e todas possíveis classes e contas pertencentes?\n [S/N] >>> ") is False:
		desc = "Operação cancelada pelo usuário."
		print(desc)
		return [ None, desc ]
	try:
...
def save_backup(contas=None, nome_arquivo='BACKUP.txt'):
	...
	if type(contas) != type(dict()):
		from manuseio import convet_xlsx_to_contas
		contas = convet_xlsx_to_contas(nome_arquivo="registro.xlsx")
	from manuseio import convert_contas_to_xlsx
	from Funcs import atual_data
	from Historico import contas_antigas
	data_atual = atual_data()
	desc = retorno = convert_contas_to_xlsx(contas=contas, nome_arquivo="registro.xlsx", verificar=True)
