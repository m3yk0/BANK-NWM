...
dir = 'START-NWM/mobile-nwm_V4_6'
complete_dir = path.abspath(path.expanduser(path.expandvars(".")))


def criar_arquivo(local='START-NWM/mobile-nwm_V4_6', nome_arquivo='registro.xlsx',
...
def localizar_arquivo(criar=False, text=False, dir='START-NWM/mobile-nwm_V4_6', arquivo='registro.xlsx', modo='rb',
...
def obter_arquivo(local=None, arquivo=None, modo='rb', pandas=False):
	...
	if arquivo is None or local is None:
		create_file, patch_atual, complete_archive, patch_esperado, correspondencia, onde_ir, arquivo_retornado = localizar_arquivo(
			criar=False, text=False, dir='START-NWM/mobile-nwm_V4_6', arquivo='registro.xlsx', modo='rb', pandas=False,
			test_open=True)
...
def valuestolist(operation=None, df=False, contas={'reino da folha': {'inu': {'meeh': [ 0, 0 ]}}},
	...
	if operation == 1:
		...
		contas = literal_eval(contas)
		vtl = valuestolist(operation=1, contas=contas)[-1]
	elif operation == 3:
		"""VTL to contas"""
		# Converte para uso:
			...
			for pos, element in enumerate(sep):
				if pos == 0:
					...
					if tipo.strip().upper() == 'REG':
						...
						item = valuestolist(operation=3, vtl=ValuesToList)[ 0 ]
					else:
						item = element.strip()
...
'''def fazer_backup(operation='r', local=None, arquivo='BACKUP.txt',
					ListBackup=[ [ 'REG', '22/05/2024-14:00:01', 'None backup', {'none': {'none': {'none': [ 'none', 'none' ]}}} ] ]):
	...
	with open('BACKUP.txt', operation) as arq:
		rl = arq.readlines()
		for pos, elem in enumerate(rl):
			print(elem)'''
