...
nome_arquivo = \
	localizar_arquivo(criar=True, text=False, dir='START-NWM/mobile-nwm_V4_6', arquivo='registro.xlsx',
...
def main():
	...
	def alt(escolha, contas=contas):
		...
		if '/' in escolha and "-" in escolha:
			...
			if i in alternativas:
				return run(contas_param=contas, command=escolha)
			else:
				print(f">> Comando '{i}' inválido. Tente novamente. ")
				return None
		else:
			i, *args = escolha.split(' ')
			return run(i, contas_param=contas)
	...
	while True:
		...
		print(f"\nNOTAS:\n1-(Escolha o número ou digite o comando como /OPERAÇÃO -parametro)\n2-(-Name == Nome da "
			  ...
			  f"linha de comando para forcar acesso mesmo em opções bloqueadas (cuidado)).\n 4- {'(Você está manuseando um arquivo não salvo)' if Usages.validar(contas=contas, nome_arquivo=nome_arquivo) is True else '(A variável contas_param atual está salva)'}\n\nMenu {loops} ",
			  end="")
		...
		if "&&" in escolha:
			...
			for cmd in commas:
				...
				if "/" in cmd and "-" in cmd:  # Caso seja comando com argumentos:
					retorno = alt(escolha, contas=contas)
