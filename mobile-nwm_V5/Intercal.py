...
def pib_real_fc(Itens=convert_things_to_items(), change=True):
	from controles import pib_real_fc
	return pib_real_fc(Itens, change)

def values_fc(registro):
	from controles import values_fc
	return values_fc(registro=registro)

def capta_fc(registro):
	from controles import capta_fc
	return capta_fc(registro=registro)
...
def zero2(algo):
    	...
    	for c in range(1, v):
    		zero_ += "0"
    	if algo.replace('.', '').replace(',', '').isnumeric():
    		zero_ = int(zero_)
    	else:
    		print("ERRO 'FUNC ZERO2(), Por favor, bote NUMERIC VALUE como argumento.")
    	return zero_
